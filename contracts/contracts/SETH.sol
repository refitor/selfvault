...
contract SETH is ZamaEthereumConfig {
    ...
    function requestWithdraw(uint128 amount) public {
        ...
        FHE.allowThis(privData.waitWithdrawAmount);
        FHE.allow(privData.waitWithdrawAmount, msg.sender);
    }

    // @notice privateWithdraw is used to withdrawal ETH from the SETh contract, demo: SETH === send 1 ETH ===> cold wallet.
    function privateWithdrawForCold(bytes memory signature, bytes32 message, uint128 amount, uint256 nonce, bytes memory publicDecryptionProof) public {
        address cold = keySignatureVerify(signature, message);
        bytes32 coldHash = _coldHash(cold);
    ...
    function _coldHash(address walletAddress) pure private returns (bytes32) {
        bytes32 typeHash = keccak256(abi.encodePacked('string Action'));
        bytes32 valueHash = keccak256(abi.encodePacked(abi.encodePacked(walletAddress)));
        return keccak256(abi.encode(typeHash, valueHash));
    }
     ...
     * @param message random string dynamically generated by the front end.
     */
    function keySignatureVerify(bytes memory signature, bytes32 message) pure public returns (address) {
    ...
    function splitSignature(bytes memory sig) pure public returns (bytes32 r, bytes32 s, uint8 v) {
