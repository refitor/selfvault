    ...
    <div>
        ...
        <Modal v-model="popModal" :footer-hide="hideFooter" class-name="vertical-center-modal">
            ...
            <div v-if="modalMode === 'Sign'">
                <Input v-model="signMessage" type="text" style="margin-top: 20px;"
                placeholder="Enter the timestamp (milliseconds) of the offline signing."><span slot="prepend">Message</span></Input>
    ...
    export default {
        ...
        data() {
            return {
                ...
                contractName: "SETH",
                selfETH: {
                    "0xaa36a7": "0x54c0D3A5D8c6eeE41B2a17247Aaaba97f79d4306",
                    "ABI": SelfETHABI,
                    contract: {}
                },
                formatTx: "",
        ...
        methods: {
            ...
            async withdraw() {
                ...
                if (this.signMessage === "") {
                    ...
                    const strMessage = `${this.signTimestamp}`;
                    this.signMessage = await this.selfETH.contract.getMessageHash(strMessage);
                ...
                const msgHash = this.signMessage;
                this.signMessage = "";
                const walletAddress = ethers.getAddress(this.wallet.getAccount());
                // const ciphertexts = await this.encrypt(coldAddress);
                ...
                const weamount = await this.selfETH.contract.waitWithdrawAmount();
                const decryptedResults = await window.fheInstance.publicDecrypt([weamount]);
                console.log("waitWithdrawAmount:", weamount, "decryptedResults:", decryptedResults.decryptionProof);
                ...
                if (this.hotSign) {
                    ...
                    const gasLimit = await this.selfETH.contract.privateWithdrawForCold.estimateGas(this.offlineSignature, msgHash, ethers.parseEther(this.inputAmount), BigInt(this.signTimestamp), decryptedResults.decryptionProof);
                    let options = { from: walletAddress, gasLimit: BigInt(gasLimit), gasPrice: (await this.wallet.provider.getFeeData()).gasPrice };//, nonce: nonce}
                    const sentTx = await this.selfETH.contract.privateWithdrawForCold(this.offlineSignature, msgHash, ethers.parseEther(this.inputAmount), BigInt(this.signTimestamp), decryptedResults.decryptionProof, options);
                    await sentTx.wait(1);
                    return sentTx.hash;
                }
            },
            ...
            async offlineSign() {
                let self = this;
                if (this.signMessage === "" || this.offlineSignature === "") {
                    this.$Message.warning('Please enter the generated offline signature');
                    return "";
                }
                this.$Modal.confirm({
